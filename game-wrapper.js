// ===== Game Configuration =====
// Game configuration maps game paths to their metadata
const GAME_CONFIG = {
    'blockmerge': {
        name: 'BlockMerge',
        path: 'builds/blockmerge/game/index.html',
        description: 'Merge blocks to create higher numbers and reach the highest score!',
        loadingTitle: 'Loading BlockMerge...'
    }
    // Add more games here as they're added
    // 'game2': {  name: 'Game 2', path: 'builds/game2/game/index.html', ... }
};
...
function getCurrentGame() {
    ...
    return gameName ? GAME_CONFIG[gameName] : null;
}
...
function init() {
    ...
    if (!currentGame) {
        // No game specified, redirect to hub
        window.location.href = 'index.html';
        return;
    }

    // Set game metadata
    document.title = currentGame.name + ' - Unity WebGL Game';
    elements.gameName.textContent = currentGame.name;
    elements.gameTitle.textContent = currentGame.name + ' - Unity WebGL Game';
    elements.loadingTitle.textContent = currentGame.loadingTitle;
    if (elements.instructionText) {
        elements.instructionText.textContent = currentGame.description;
    }

    // Load the game
    elements.gameFrame.src = currentGame.path;
